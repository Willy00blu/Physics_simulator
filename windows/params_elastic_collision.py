...
class ElasticCollisionParams(tk.Frame):
    ...
    def _create_widgets(self):
        tk.Label(self, text="Elastic Collision", font=("Arial", 16, "bold")).grid(row=0, columnspan=2, pady=10)
        ...
        tk.Label(self, text="Shape:").grid(row=7, column=0, sticky="e", pady=(10, 0))
        shape_row = tk.Frame(self)
        shape_row.grid(row=7, column=1, sticky="w", pady=(10, 0))
        ...
        tk.Button(self, text="Back",  command=self.on_back).grid(row=8, column=0, pady=12)
        tk.Button(self, text="Start", command=self._submit).grid(row=8, column=1, pady=12)
